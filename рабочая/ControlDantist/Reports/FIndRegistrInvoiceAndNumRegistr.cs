...
{
    public class FIndRegistrInvoiceAndNumRegistr: Registr,IReportingRegistrInvoiceAndNumRegistr
    {

        // Переменная для хранения количество адоговоров.
        private int iCountD = 0;
        /// <summary>
        /// Поиск по счет-фактуре и по номеру реестра.
        /// </summary>
        /// <param name="num_счётФактура"></param>
        /// <param name="dataStart"></param>
        /// <param name="dataEnd"></param>
        /// <returns></returns>
        public DataTable FindInvoiceAndNumRegistr(string num_счётФактура, string numReestr, string dataStart, string dataEnd)
        {
            ...
            string query = "SELECT АктВыполненныхРабот.id_акт,АктВыполненныхРабот.НомерАкта,АктВыполненныхРабот.НомерРеестра, " +
                ...
                //" (АктВыполненныхРабот.НомерРеестра = '" + unmReestr + "') " +
                    "and ((АктВыполненныхРабот.НомерРеестра = '" + numReestr + "') and (АктВыполненныхРабот.НомерСчётФактуры = '" + num_счётФактура + "')) " +
                //"and (АктВыполненныхРабот.НомерСчётФактуры = '" + num_счётФактура + "') " +
                    ...
                    "dbo.Льготник.id_льготник,dbo.Льготник.id_район,dbo.Льготник.id_насПункт,Льготник.улица,Льготник.НомерДома,Льготник.корпус,Льготник.НомерКвартиры,Договор.ДатаРеестра,Договор.ДатаАктаВыполненныхРабот ";
