...
{
    public class FindPerson_FI : IFindPerson
    {
        private string фамилия = string.Empty;
        private string имя = string.Empty;

        public FindPerson_FI(string фамилия, string имя)
        {
        ...
        public string Query()
        {
           ...
           (
                ...
                HAVING(WM_PERSONAL_CARD.A_STATUS = 10) AND " +
                      //(WM_PERSONAL_CARD.A_PCSTATUS = 1) and  "  +
                      "LTRIM(RTRIM(SPR_FIO_SURNAME.A_NAME)) = '" + this.фамилия.ToLower().Trim() + "' " +
