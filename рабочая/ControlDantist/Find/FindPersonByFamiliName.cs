...
{
    class FindPersonByFamiliName : IFindPerson
    {
        // Переменные для хранения Фамилии и имении.
        private string firstName, name;

        public FindPersonByFamiliName(string firstName, string name)
        {
        ...
        public string Query()
        {
            string query = @"declare @FirstName nvarchar(150)
                              ...
                              inner join Льготник
                                ...
                                and(ДатаАктаВыполненныхРабот is not null) and(YEAR(ДатаАктаВыполненныхРабот) <> 1900) and АктВыполненныхРабот.ДатаПодписания is not null
                    ...
                    where Фамилия = @FirstName and Имя = @Name and ДоговорAdd.ФлагПроверки = 1 and АктВыполненныхРаботAdd.ДатаПодписания is not null
                    ...
                    where Фамилия = @FirstName and Имя = @Name and (Договор.idFileRegistProgect > 0 or Договор.idFileRegistProgect = 0 ) and Договор.ФлагПроверки = 1 and Договор.flagОжиданиеПроверки = 1
                    and АктВыполненныхРабот.ДатаПодписания is not null and YEAR(Договор.ДатаЗаписиДоговора) > 2019
                    group by Договор.id_договор, НомерДоговора, Льготник.Фамилия, Льготник.Имя, Льготник.Отчество, ЛьготнаяКатегория.ЛьготнаяКатегория,
        ...
        //                                where TabPerson.Фамилия = @FirstName and TabPerson.Имя = @Name and Tab1.ФлагПроверки = 1
        //union
        ...
        //                                where TabPerson.Фамилия = @FirstName and TabPerson.Имя = @Name and TabPerson.ФлагПроверки = 1 and TabPerson.DateWriteLetter is not null";
        //            //--and TabPerson.ДатаРождения = @DateBirth and
        //            //--TabPerson.НомерДоговора<> @NumContract and YEAR(Tab1.ДатаПодписания) <> 1900 "
