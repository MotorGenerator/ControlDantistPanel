    ...
    public partial class FormDisplayReestr : Form
    {
        ...
        private void FormDisplayReestr_Load(object sender, EventArgs e)
        {
            ...
            //Пройдёмся по DataGrid и псочитаем суммы договоров и суммы актов
             ...
             this.labelFile.Text = "Сумма реестра выполненных договоров " + Math.Round(sumРеестрАкт,2).ToString("c");
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (this.АктыРабот.ContainsKey(НомерАкта))
            {
                ...
                if (dr.ErrorPerson == true)
                {
                   // MessageBox.Show("Изменены данные о льготнике");
                    ...
                    if (message.DialogResult == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        //IEnumerable<Unload> un = this.Unloads.Where(u => u.Льготник.Rows[0]["Фамилия"] == "Агафонова").Select(u=>u);
                        IEnumerable<Unload> un = this.Unloads.Where(u => u.Льготник.Rows[0]["Фамилия"].ToString() == фамилия[0].Trim()).Select(u => u);
                ...
                if (dr.СписокОшибок != null)
                {
                    ...
                    //this.dataGridView2.Columns["Количество"].Width = 100;
                    ...
                    if (listCorrect.Count != 0)
                    {
                        ...
                        //this.dataGridView3.Columns["Количество"].Width = 100;
            ...
            //DisplayReestr dr =  this.АктыРабот[НомерДоговора]
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            string logTest = MyAplicationIdentity.GetUses();
             ...
             // Проверим все ли договора в реестре прошли проверку
            ...
            if (countContracts == countD)
            {
                ...
                string date_СчётФактуры = this.ДатаСчётФактуры;// formR.ДатаСчётФактуры;
                    ...
                    foreach (DataGridViewRow row in this.dataGridView1.Rows)
                    {
                        ...
                        //row.Cells["НомерАкта"].Value
                        //row.Cells["НомерДоговора"].Value
                        //DisplayReestr act = this.АктыРабот[keyAct];
                        ...
                        foreach (Unload un in this.Unloads)
                        {
                            ...
                            if (numDog == номерДоговора && flag == true)
                            {
                                ...
                                //Получим id договора текущего договора
                                string query = "select * from dbo.Договор " +
                                               "where НомерДоговора = '" + номерДоговора + "' and ФлагПроверки = 'True' ";
                                ...
                                if (Convert.ToInt32(tab.Rows[0]["Количество"]) == 0)
                                {
                                    ...
                                    string updateLgot = " UPDATE [Льготник] " +
                                                        ...
                                                        ",[НомерДома] = '" + rowЛьгот["НомерДома"].ToString().Trim() + "' " +
                                                        ...
                                                        ",[КемВыданДокумент] = '" + rowЛьгот["КемВыданДокумент"].ToString().Trim() + "' " +
